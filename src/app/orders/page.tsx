...
export default function OrdersPage() {
  ...
  const [sortKey, setSortKey] = useState<
    "created_at" | "status" | "size" | "total_price" | null
  >("created_at");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");
  const [statusFilter, setStatusFilter] = useState<string | null>(null);
  const [statusSortDirection, setStatusSortDirection] = useState<
    "asc" | "desc"
  >("asc");
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const productMatch =
      !productFilter ||
      order.size.name.toLowerCase() === productFilter.toLowerCase();
  ...
  const sortedOrders = [...filteredOrders].sort((a, b) => {
    ...
    if (sortKey === "status") {
      const aIndex = statusOrder.findIndex(
        (status) => status.toLowerCase() === a.status.toLowerCase()
      );
      const bIndex = statusOrder.findIndex(
        (status) => status.toLowerCase() === b.status.toLowerCase()
      );
      return statusSortDirection === "asc" ? aIndex - bIndex : bIndex - aIndex;
    }
    if (sortKey === "size") {
      const aIndex = sizeOrder.findIndex(
        (size) => size.toLowerCase() === a.size.name.toLowerCase()
      );
      const bIndex = sizeOrder.findIndex(
        (size) => size.toLowerCase() === b.size.name.toLowerCase()
      );
      if (aIndex === bIndex) {
